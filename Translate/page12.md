...
Давайте поговорим о некоторых операциях, которые он *имеет*; Я пока опущу некоторые, которые не используются в Hello World.
...
(Если вы не уверены, "to increment" означает "добавить 1", а "to decrement" означает "вычесть 1".)
Так, например, `dec bc` в строке {{#line_no_of "^\s*dec bc" ../assets/hello-world.asm}} из `hello-world.asm` просто вычитает 1 из `bc`.

Ладно, круто!
Но не могли бы мы идти немного быстрее?
Конечно, мы можем, с помощью `add` и `sub'!
Они соответственно добавляют(ADD) и вычитают(SUBtract) произвольные значения (либо константу, либо регистр).
Ни то, ни другое не используется в руководстве, но братом `sub` является: заметили ли вы маленькую "cp" в строке {{#line_no_of "^\s*cp\b" ../assets/hello-world.asm}}?
...
-----|---------------------
  ...
  H  | Half-carry(Полуподдержка)
  C  | Carry(Поддержка)

Да, есть флаг с именем "C" и регистр с именем "c", и **это разные, не связанные между собой вещи**.
Это делает синтаксис немного запутанным в начале, но они всегда используются в разных контекстах, так что все в порядке.
...
C устанавливается, когда операция *переполняется* или *недонаполняется*.

Что такое переполнение?
Давайте возьмем простую инструкцию `add a, 42`.
Это просто добавляет 42 к содержимому регистра `a` и записывает результат обратно в `a`.
...
Когда выполняется операция, устанавливается carry флаг, если произошел overflow или underflow, очищает флаг в противном случае.
...
Хорошо, проверка на равенство - это хорошо, но мы также можем захотеть выполнить *сравнения*.
Не волнуйтесь, потому что флаг переноса здесь именно для этого!
Видите ли, при выполнении вычитания флаг переноса устанавливается, когда результат опускается ниже 0 — но есть просто способ сказать "становится отрицательным"!
