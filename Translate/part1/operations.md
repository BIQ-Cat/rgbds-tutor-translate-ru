...
Хорошо, мы знаем, как передавать значения, но просто копировать числа - это скучно; мы хотим их изменять!

Процессор Game Boy не обеспечивает наличие всех операций (например, нет инструкции умножения), но мы можем просто запрограммировать их сами с помощью того, что у нас есть.
Давайте поговорим о тех операциях, которые у него есть (я пока опущу некоторые из них, которые не используются в Hello World).
...
Ни то, ни другое не используется в руководстве, зато используется инструкция `cp`, по функционалу близкая к `sub`. Он находится в строке {{#line_no_of "^\s*cp\b" ../assets/hello-world.asm}}.
`cp` позволяет сравнивать(to ComPare) значения.
`cp` работает так же, как и `sub`, но он отбрасывает результат вместо того, чтобы записывать его.
"Подожди, так он ничего не делает?" - вы можете спросить. Нет, он обновляет **флаги**.

## Флаги

Пришло время поговорить о регистре специального назначения (помните такие?) `f`, который используется для *флагов*.
...
C устанавливается, когда результат *переполняется*(overflows) или *опускается ниже нуля*(underflows).
...
Ну, можно было бы подумать, что `a` будет равно 220 + 42 = 262, но это не так.
...
Теперь попробуем *сравнить* числа.
Для этого как раз и нужен флаг С.
Видите ли, при выполнении вычитания флаг переноса устанавливается, когда результат опускается ниже 0 — то есть становится отрицательным!

Итак, когда устанавливается флаг carry, мы знаем, что `a - операнд < 0`, следовательно, `a < операнд`..!
