...
Например, мы используем один и тот же алгоритм для копирования в памяти в трех местах.
...
Наша же функция завершается тогда и только тогда, когда мы используем инструкцию `ret`.
...
В assembly у функций нет привычных нам _параметров_, поэтому эти комментарии очень важны для описания её работы.
