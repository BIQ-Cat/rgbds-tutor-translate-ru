...
Итак, теперь мы можем двигать ракетку влево-вправо!
Однако для полноценной игры нам нужен ещё один объект: мячик.
...
Этот метод обращения к объекту может показаться кустарным, однако на данный момент у нас совсем мало объеков, поэтому можем пока оставить все как есть.
В будущем работать с OAM будет гораздо проще.
...
Так как нам придется проверять коллизию несколько раз, мы напишем для этого функцию.
...
Если тайл совпадает (функция находит стену), она завершается, устанавливая флаг `Z`.
`ret z` позволяет нам завершать выполнение функции только при условии, что установлен флаг `Z` (иначе функция продолжит выполнение).
Таким образом, если мы встречаем тайл стены, функция завершится с установленным флагом `Z`; в ином случае флаг `Z` по окончании функции будет снят.
...
Однако вы, наверное, также заметили, что мы вычитаем дополнительный пиксель из координаты Y.
...
## Бонус: исправление высоты отскока
...
Либо вы можете добавить `sub a, 6` сразу после `ld a, [STARTOF(OAM)]`.
