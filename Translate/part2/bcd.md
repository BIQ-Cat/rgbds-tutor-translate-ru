...
Поскольку мы ограничены в использовании памяти, мы выделим для хранения счёта всего один байт. Существует множество способов чтения и записи десятичных чисел, но в этот раз мы воспользуемся так называемым "Packed Binary Coded Decimal" (Упакованный двоично-десятичный код) или packed BCD.
...
Помните, как работают системы счисления? Не [забывайте освежать свою память](../part1/bin_and_hex.md), если вы испытывайте с ними сложности.
...
Для начала мы запишем адрес `wScore` в HL, чтобы в дальнейшем иметь возможность производить операции наподобие `LD A, [HL]` и `LD [HL], A`. Это на 1 такт и на 1 байт эффективнее, чем если бы мы просто писали адрес в `ld` (`LD A, [wScore]` и `LD [wScore], A`).

Далее мы загружаем счет в регистр `a` - единственный регистр, который работает с `ADD`. После этого действия мы сможем воспользоваться этой инструкцией, чтобы увеличить счёт на единицу.
...
:::tip Почему `add 1` вместо `inc`

Дело в том, что `inc` НЕ изменяет флаг Carry, в отличие от `add 1`. `daa` использует флаг Carry для того, чтобы корректировать наше десятичное число. Соответственно, при использовании `inc` вышеприведённый код _не будет работать_.
...
Счёт должен увеличиваться только после разрушения блока. Добавим инструкцию `call IncreaseScorePackedBCD` после обработки коллизии блоков (имеется ввиду коллизия **и с левым, и правым тайлом блока**) в `CheckAndHandleBrick`. 
...
Перед тем, как мы сможем отобразить наш счет, нам необходимо добавить графическое представление цифр 0-9. У нас уже имеются тайлы для этого проекта в [данном файле](https://github.com/gbdev/gb-asm-tutorial/raw/master/unbricked/bcd/digit-tileset.asm). Вставьте информацию прямо перед меткой `TilesEnd`. Тайлы должны выглядить так:
...
В начале игры мы будем отображать 2 нуля (тайл с id `$1A`). Давайте разместим их на третей строке, начиная с 4 тайлов слева.
...
Вы можете найти адрес тайла внутри VRAM в tile map viewer'е вашего эмулятора.
